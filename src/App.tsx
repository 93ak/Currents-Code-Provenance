...
export default function App() {
  ...
  const [darkMode, setDarkMode] = useState<boolean>(() => {
    try {
      return localStorage.getItem('iet-dark-mode') === 'true';
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem('iet-dark-mode', String(darkMode));
    } catch { /* ignore */ }
  }, [darkMode]);
  ...
  const handleLikeProject = async (projectId: string) => {
    ...
    try {
      ...
      if (res.success && res.project) {
        ...
        showToast(res.liked ? 'Project starred!' : 'Star removed.', 'success');
      }
    } catch {
      showToast('Error updating project star. Please try again.', 'error');
    }
  };
  ...
  if (authChecking) {
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center p-4 ${darkMode ? 'bg-slate-900' : 'bg-slate-900'}`}>
        <Loader2 className="w-8 h-8 text-[#9b51e0] animate-spin mb-3" />
        <p className="text-sm font-semibold tracking-wide font-['Poppins'] text-white">Connecting to IET Portal Backend...</p>
      </div>
    );
  }

  return (
    <div className={`min-h-screen flex flex-col overflow-x-hidden transition-colors duration-300 ${darkMode ? 'bg-[#0f0f13] text-slate-100' : 'bg-slate-50 text-slate-900'} font-sans`}>
      ...
      <div className="flex flex-1 relative">
        ...
        <main className="flex-1 p-6 sm:p-8 lg:p-10 overflow-y-auto max-w-7xl mx-auto w-full">
          {activeTab === 'auth' && (
            <AuthView onAuthSuccess={handleAuthSuccess} darkMode={darkMode} />
          )}
