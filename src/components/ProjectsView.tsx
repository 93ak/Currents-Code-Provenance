    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-3xl border shadow-sm ${card}`}>
        <div>
          ...
          <p className={`text-xs mt-1 ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>Explore and appreciate engineering builds by IET CONNECT chapter members</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className={`rounded-3xl max-w-lg w-full p-6 space-y-4 relative shadow-2xl border max-h-[90vh] overflow-y-auto ${card}`}>
            ...
            <h2 className={`text-lg font-bold font-['Poppins'] ${darkMode ? 'text-slate-100' : 'text-slate-900'}`}>
              Submit Member Project
            </h2>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className={`block text-xs font-semibold mb-1 ${labelCls}`}>Project Title *</label>
                  <input type="text" required value={newProjData.title} onChange={(e) => setNewProjData({ ...newProjData, title: e.target.value })} placeholder="e.g. Smart Solar Grid Monitor" className={`w-full px-3 py-2 border rounded-xl text-xs outline-none ${inputCls}`} />
                </div>
                <div className="col-span-2">
                  <label className={`block text-xs font-semibold mb-1 ${labelCls}`}>Short Tagline</label>
                  <input type="text" value={newProjData.tagline} onChange={(e) => setNewProjData({ ...newProjData, tagline: e.target.value })} placeholder="e.g. Real-time IoT solar efficiency dashboard" className={`w-full px-3 py-2 border rounded-xl text-xs outline-none ${inputCls}`} />
                </div>
                <div>
                  <label className={`block text-xs font-semibold mb-1 ${labelCls}`}>Domain & Field</label>
                  <select value={newProjData.domain} onChange={(e) => setNewProjData({ ...newProjData, domain: e.target.value as any })} className={`w-full px-3 py-2 border rounded-xl text-xs outline-none ${inputCls}`}>
                ...
                <div>
                  <label className={`block text-xs font-semibold mb-1 ${labelCls}`}>Status</label>
                  <select value={newProjData.status} onChange={(e) => setNewProjData({ ...newProjData, status: e.target.value as any })} className={`w-full px-3 py-2 border rounded-xl text-xs outline-none ${inputCls}`}>
                ...
                <div className="col-span-2">
                  ...
                  <textarea rows={3} required value={newProjData.description} onChange={(e) => setNewProjData({ ...newProjData, description: e.target.value })} placeholder="Explain architecture, technology stack, problem solved..." className={`w-full px-3 py-2 border rounded-xl text-xs outline-none resize-none ${inputCls}`} />
                </div>
                <div className="col-span-2">
                  <label className={`block text-xs font-semibold mb-1 ${labelCls}`}>GitHub Repository Link *</label>
                  <input type="url" required value={newProjData.githubUrl} onChange={(e) => setNewProjData({ ...newProjData, githubUrl: e.target.value })} placeholder="https://github.com/username/repo" className={`w-full px-3 py-2 border rounded-xl text-xs outline-none ${inputCls}`} />
                </div>
                <div className="col-span-2">
                  <label className={`block text-xs font-semibold mb-1 ${labelCls}`}>Live Demo Link (Optional)</label>
                  <input type="url" value={newProjData.demoUrl} onChange={(e) => setNewProjData({ ...newProjData, demoUrl: e.target.value })} placeholder="https://my-app.example.com" className={`w-full px-3 py-2 border rounded-xl text-xs outline-none ${inputCls}`} />
                </div>
                <div className="col-span-2">
                  <label className={`block text-xs font-semibold mb-1 ${labelCls}`}>Team Members (Comma separated)</label>
                  <input type="text" value={newProjData.teamMembersStr} onChange={(e) => setNewProjData({ ...newProjData, teamMembersStr: e.target.value })} placeholder="John, Sarah, Priya" className={`w-full px-3 py-2 border rounded-xl text-xs outline-none ${inputCls}`} />
                </div>
              </div>

              <div className={`flex justify-end gap-3 pt-4 border-t ${darkMode ? 'border-slate-700' : 'border-slate-100'}`}>
                <button type="button" onClick={() => setShowSubmitModal(false)} className={`px-4 py-2 rounded-xl text-xs font-bold ${darkMode ? 'bg-slate-700 text-slate-200' : 'bg-slate-100 text-slate-600'}`}>Cancel</button>
                <button type="submit" className="px-5 py-2 text-xs font-bold text-white bg-[#622569] hover:bg-[#9b51e0] rounded-xl shadow transition-all">Submit Project</button>
              </div>
            </form>
          </div>
        </div>
